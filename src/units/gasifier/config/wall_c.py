"""
@module gasifier.config.wall_c
@brief Wall structural configuration for the gasifier (shell-tube mode).

@details
Builds the `wall_config` dict that activates the dynamic wall model
Tw(z, t) in the gasifier ODE. When `wall_config` is present in `params`,
the state vector grows by N degrees of freedom (wall temperature per cell).

Self-contained implementation — no dependency on heater or adsorber modules,
so the gasifier remains functional if those units are refactored or removed.
...
Output keys in wall_config:
    ...
    epsilon_wall : float       — inner wall surface emissivity [-]
                   Reserved for future gas-wall / particle-wall radiation terms.
...
def build_wall_config(
    ...
    """
    Build the structural wall configuration for the gasifier shell-tube model.
    ...
    epsilon_wall  : float  inner wall surface emissivity [-] (default 0.85).
                   Reserved for future radiation terms (gas-wall, particle-wall).
