...
The operating mode (batch / cstr / prf) is read from bc_config["mode"] and
determines what values are computed:

    "batch" — v_in = v_out = 0; no inlet composition or temperature.
    "cstr"  — v_in prescribed; v_out from molar continuity; inlet C, T prescribed.
    "prf"   — identical BC logic to "cstr"; spatial behaviour differs via N and
              axial dispersion, not here.

This is the only module that evaluates BC values at time t. The RHS receives
the returned dict and is agnostic to the mode.
"""
...
def get_reactor_boundary(
    t:          float,
    P_cell:     np.ndarray,   # (N,) [bar]  gas pressure in cells
    Ctot_cell:  np.ndarray,   # (N,) [mol/m³_gas]  total molar concentration
    ...
    dict with keys:
        inlet : {
            ...
            y        : ndarray(nc,) or None  molar fractions [-]
            C_mol_m3 : ndarray(nc,) or None  concentrations [mol/m³_gas]
        }
        outlet : {
            P_out_bar : float             outlet reference pressure [bar]
            v_m_s     : float             superficial velocity at outlet face [m/s]
        }
    """
    mode = bc_config["mode"]
    ...
    # cstr / prf: inlet prescrito, outlet por continuidad molar
    ...
    # v_out garantiza conservación molar global: v_in * Ctot_in = v_out * Ctot_out
