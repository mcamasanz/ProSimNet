...
# =============================================================================
    def __init__(self,
        ...
        #Design
        self._L      = Longitud    # Longitud total (m)
        self._D      = Diametro    # Diámetro total (m)
        self._Ri     = self._D / 2
        self._e      = Espesor     # Espesor (m)
        ...
        self._case = None
        self._steup_vars=[ '_name', '_L', '_D', '_Ri', '_e', '_vol',
    ...
    def __str__(self):
        return (f"Tank {self._name} | P0={self._P0} Pa | T0={self._T0} K | x0={self._x0} | "
                f"V={self._Vol:.3f} m³ | N0={self._N0:.3f} mol | ncomp={self._ncomp} "
                f"| nodos={self._nodos}")   
...
# =============================================================================
    ...
    def _croopTime_(self, target_time):
        ...
        - Tras recortar, el estado interno (`_state_vars`) se actualiza al último valor disponible antes de `target_time`.
        ...
        self._state_vars['t'] = self._actualTime
        self._state_vars['N'] = self._N[-1, :].copy()
        self._state_vars['T'] = self._T[-1, :].copy()
        self._state_vars['P'] = self._P[-1, :].copy()
        self._state_vars['x'] = self._x[-1, :, :].flatten().copy()
    
        # Borra logs y resultados temporales para garantizar limpieza de memoria y resultados
        self._reset_logs_()
    ...
    def _readData_(self,data): #EN DESARROLLO
        ...
        if 'state_N' in data_dict:
            if self._state_vars is None:
                self._state_vars = {}
            self._state_vars['N'] = np.array(data_dict['state_N'])
        if 'state_P' in data_dict:
            self._state_vars['P'] = np.array(data_dict['state_P'])
        if 'state_T' in data_dict:
            self._state_vars['T'] = np.array(data_dict['state_T'])
        if 'state_x' in data_dict:
            self._state_vars['x'] = np.array(data_dict['state_x'])
...
# =============================================================================
    ...
    def _get_State_ylocal_(self):
        ...
        Extraigo de `self._state_vars` los valores actuales de número de moles, fracciones molares y temperaturas de cada nodo, y los organizo en el orden requerido para la integración ODE.
        ...
        - Si `self._state_vars` no está sincronizado (por ejemplo, después de modificar manualmente variables internas), el vector puede no reflejar el estado físico real.
        ...
        ylocal = []
        x=self._state_vars["x"].reshape(self._nodos,self._ncomp)
        for idx, label in enumerate(self._labelVars):
            if label.startswith("N_"):
                nodo = int(label.split("_")[1])
                ylocal.append(self._state_vars["N"][nodo])
            elif label.startswith("T_"):
                nodo = int(label.split("_")[1])
                ylocal.append(self._state_vars["T"][nodo])
            elif label.startswith("x"):
                especie, nodo = label[1:].split("_")
                especie = int(especie)
                nodo = int(nodo)
    ...
    def _get_StateVar_(self, var, nodo=None, port=None):
        ...
        Avisos
        ------
        - El parámetro `var` debe ser una clave de self._state_vars: 'N', 'T', 'P', 'x'.
...
# =============================================================================
    def _storeData_(self, t, ylocal):
        ...
        - Actualiza el estado actual del tanque (`self._state_vars`) al último instante disponible.
    ...
    def _writeCase_(self,): # EN DESARROLLO !!!
        steup_vars = self._steup_vars
        setup_dict = {var: getattr(self, var, None) for var in steup_vars}
        setup_dict['conex'] = {}
    ...
    def _writeData_(self,): # EN DESARROLLO !!!
        ...
        data_dict['state_N'] = self._state_vars['N'].copy() if self._state_vars is not None else None
        data_dict['state_P'] = self._state_vars['P'].copy() if self._state_vars is not None else None
        data_dict['state_T'] = self._state_vars['T'].copy() if self._state_vars is not None else None
        data_dict['state_x'] = self._state_vars['x'].copy() if self._state_vars is not None else None
